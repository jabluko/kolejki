...
public:
    ...
    inline list<T>::iterator::iterator
		(node *const current, bool direction, bool const_linking)
	...
	template <class T>
	inline list<T>::list()
		: _before_first(make_node(), 1, std::is_const_v<decltype(*this)>),
		  _past_last(make_node(), 1, std::is_const_v<decltype(*this)>)
